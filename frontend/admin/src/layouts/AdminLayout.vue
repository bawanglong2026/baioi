...
<template>
  <div class="min-h-screen bg-background text-foreground">
    <div class="flex min-h-screen">
      ...
      >
        <div class="flex flex-col justify-center box-border" :class="[sidebarCollapsed ? 'px-2 h-[65px] py-0' : 'px-6 h-[76px] py-6']">
          ...
          <div v-if="sidebarCollapsed" class="text-sm font-semibold tracking-tight text-center">Baioi</div>
        </div>
        ...
        <div class="border-t border-border">
          <div v-if="!sidebarCollapsed" class="px-6 py-3 text-[11px] text-muted-foreground space-y-1">
            <p>© {{ new Date().getFullYear() }} Baioi <span v-if="appVersion" class="text-muted-foreground/70">{{ appVersion }}</span></p>
          </div>
          <button
            type="button"
